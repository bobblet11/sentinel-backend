...
def backup_jobs(all_jobs: List[StreamMessage], all_article_urls: Set[str]):
    ...
    backup_data = {"jobs": job_backup_data, "set_urls": list(all_article_urls)}

    with open(BACKUP_FILE, "w", encoding="utf-8") as f:
        json.dump(backup_data, f, ensure_ascii=False, indent=2)

    print(
        f"[INFO] Backup complete: {len(backup_data['jobs'])} jobs saved, {len(backup_data['set_urls'])} set urls saved"
    )
...
def restore_jobs(backup_file: str, merged: redis.Redis):
    ...
    print(
        f"[INFO] Restore complete: {len(backup_data['jobs'])} jobs, {len(backup_data['set_urls'])} set urls"
    )
...
def load_stream_jobs(
    port, stream_keys: List[str], host="localhost"
) -> List[Dict[str, Any]]:
        ...
        for redis_id, fields in entries:
            if "payload" in fields and isinstance(fields["payload"], str):
                try:
                    ...
                    print(
                        f"[❌] Could not parse payload JSON for {redis_id} in {key}: {e}"
                    )
                    continue

            jobs.append({"stream": key, "redis_message_id": redis_id, "data": fields})
...
def transform_job(raw_msg: Dict[str, Any]) -> StreamMessage:
    ...
    if not article_url:
        ...
        raise RuntimeError(
            f"Malformed job: missing article_url\n{json.dumps(raw_msg, indent=2)}"
        )
        return None
    ...
    if created_at_raw:
        try:
            ...
            print(
                f"[WARN] Invalid created_at format for {article_url}, falling back to now()"
            )
...
print(
    f"[INFO] Writing {len(all_jobs)} jobs and {len(article_urls_seen)} URLs to merged Redis..."
)
