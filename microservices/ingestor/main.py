...
def exec():
    ...
    available_structured_rss_feeds = [
        outlet for outlet in structured_rss_feeds if len(outlet["feeds"]) > 0
    ]
    free_outlets = [
        outlet
        for outlet in available_structured_rss_feeds
        if outlet["paywall"] == "None"
    ]
    num_free_feeds = sum(len(outlet["feeds"]) for outlet in free_outlets)
    metered_outlets = [
        outlet
        for outlet in available_structured_rss_feeds
        if outlet["paywall"] == "Metered"
    ]
    ...
    print(
        f"Number of Metered Outlets: {len(metered_outlets)} ({num_metered_feeds} feeds) [{100 * (num_metered_feeds / num_feeds)}]"
    )
