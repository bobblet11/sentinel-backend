...
class BaseIngestor:
    ...
    def __init__(self, duplicate_filter=None, publisher=None):
        ...
        if duplicate_filter is None:
            self.duplicate_filter = RedisDuplicateFilter(
                REDIS_DUPLICATE_FILTER_KEY, ttl_s=0
            )
        else:
    ...
    def _log_snapshot(self) -> None:
        ...
        entry = file_data.setdefault(cycle_key, {"snapshots": []})
    ...
    def _log_stats(
        self,
        newly_seen_articles: int = 0,
        non_new_articles: int = 0,
        total_deduplicated_articles_processed: int = 0,
        total_raw_articles_fetched: int = 0,
        outlet_counts: Dict[str, Any] = {},
        cycle_duration_s: int = 0,
    ) -> None:
    ...
    def run(self) -> None:
        ...
        unseen_urls: Set[str] = set(
            self.duplicate_filter.has_many(list([a.link for a in unique_articles]))
        )
        if not unseen_urls:
            self.logger.info("Ingestion cycle: all articles already seen.")
            return
        unseen_articles: List[Article] = [
            article for article in unique_articles if article.link in unseen_urls
        ]
