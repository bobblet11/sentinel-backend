...
class RssIngestor(BaseIngestor):
    ...
    def _fetch_and_parse_feed(self, rss_url: str) -> FeedParserDict | None:
        ...
        """
        try:
            feed: FeedParserDict = parse(
                rss_url, agent=user_agent_manager.generate_profile().user_agent_string
            )
            if feed.bozo:
    ...
    def fetch_articles(
        self, max_workers: int = MAX_INGESTOR_WORKERS
    ) -> Iterator[Article]:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for feed in future_to_feed:
                ...
                self.logger.debug(
                    f"\tProcessing entries from: {feed.feed.get('title', 'Unknown Title')}"
                )
