...
from microservices.nlp.config import PHOTO_CREDIT_MAX_LEN, PREPROCESS_MIN_TOKENS
...
class Preprocessor(SentenceProcessor):
    ...
    def _clean_and_repair_structure(self, raw_text: str) -> str:
        ...
        footer_cutoff_pattern = re.compile(
            r"(?i)^("
        ...
        reuters_meta_pattern = re.compile(
            r"(?i)^("
        ...
        for line in lines:
            ...
            if (
                "/" in line
                and len(line) < PHOTO_CREDIT_MAX_LEN
                and photo_credit_pattern.search(line)
            ):
    ...
    def run(
        ...
        for idx, span in enumerate(doc.sents):
            ...
            s_obj = SentenceScore(
                index=idx, text=text_segment, score=0.0, embedding=None
            )
            sentence_objects.append(s_obj)

        logger.info(
            f"Preprocessor: Cleaned & Split. Result: {len(sentence_objects)} sentences."
        )
