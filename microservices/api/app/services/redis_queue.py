...
routing_map = RedisPublisherRouter.generate_router_mapping(
    [OUTPUT_STREAM, "background:to.be.scraped"],
    [JobType.USER.value, JobType.BACKGROUND.value],
)
...
def publish_job(job: Job, article: Article, job_dto: JobCreate) -> None:
    try:
        ...
        payload = MessagePayload(
            ...
            news_outlet=job_dto.news_outlet
            or (article.outlet.name if article.outlet else None),
            title=article_title,
            author=job_dto.article_author,
            publish_date=job_dto.article_published_at
            or (article_published_at.isoformat() if article_published_at else None),
        ...
        message = add_timestamp_to_message(
            message=message, stage_name=JobStage.INGESTED
        )
