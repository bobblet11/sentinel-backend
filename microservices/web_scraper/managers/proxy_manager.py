...
class ProxyManager:
    ...
    def __init__(
        ...
        with self._init_lock:
            ...
            self.http_proxies = set[str]()
            self.https_proxies = set[str]()
    ...
    def __perform_full_refresh(self) -> None:
        ...
        if self.http_proxies or self.https_proxies:
            ...
            tested_current_proxies = self.__validate_proxies(
                list(self.http_proxies), list(self.https_proxies)
            )
        ...
        tested_proxinet_proxies = self.__validate_proxies(
            untested_proxinet_proxies.get("http", []),
        ...
        untested_proxifly_proxies = self.__fetch_proxifly(
            proxies_round1=(
                ...
                if (http1 and https1)
                else {"http": https1, "https": https1} if (https1) else None
            ),
            proxies_round2=(
    ...
    def __validate_proxies(
        self, candidates_http: List[str], candidates_https: List[str]
    ) -> Dict[str, Set[str]]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Map HTTP tasks
            http_results = executor.map(
                lambda p: self.__test_proxy(p, "http"), candidates_http
            )
    ...
    def __scrape_proxinet_proxies(self):
        ...
        try:
            ...
            print(
                f"FreeProxyList: scraped {total} proxies (http: { len(http_proxies)}, https: {len(https_proxies)})"
            )
        ...
        return {"http": list(http_proxies), "https": list(https_proxies)}
