...
class ProxyManager:
    ...
    def __init__(
        ...
        with self._init_lock:
            ...
            self.saved_json_source = JsonFileSource("Saved Proxies", os.path.join(script_dir, "saved_proxies.json"))
            self.saved_webshareio_source = WebshareIOFileSource(os.path.join(script_dir, "webshareio_proxies.json"))
    ...
    def _load_and_validate_initial_proxies(self):    
        ...
        saved_webshareio_https_proxies = self.saved_webshareio_source.get_proxies()["https"]
    ...
    def get_proxy(self) -> ProxyRequestDict:
        """
        Public method to get a single, working proxy.
    ...
    def _perform_full_refresh(self) -> None:
        ...
        newly_fetched_proxies: ProxyDict = {"https": set(), "socks4": set(), "socks5": set()}
        
        for source in self.sources:
            print(f"[*] Fetching proxies from {source.name}...")
            bootstrap_proxy = self._create_proxy_dict(random.choice(list(bootstrap_candidates))) if bootstrap_candidates else None
        ...
        newly_fetched_proxies["https"].update(self.saved_webshareio_source.get_proxies())
    ...
    def _validate_proxies(self, candidates: ProxyDict) -> ProxyDict:
        ...
        print(f"[*] Validating {len(candidates.items())} proxies")
    ...
    def _test_proxy(self, proxy: str, p_type: str) -> Optional[Tuple[str, str]]:
        ...
        try:
            response = requests.get(self.test_url, proxies={"http": proxy_url, "https": proxy_url}, timeout=self.timeout)
...
if __name__ == "__main__":
    proxinet_source = ProxiNetHttpSource('ProxiNetHttpSource', timeout=(10,12))
    proxifly_source = ProxiflyHttpSource('ProxiflyHttpSource', timeout=(10,12))
    ...
    for i in range(5):
        ...
        if proxy:
            print(f"[{i+1}/5] [SUCCESS] Got Proxy: {proxy}")
        else:
            print(f"[{i+1}/5] [FAILURE] Could not retrieve a proxy.")
