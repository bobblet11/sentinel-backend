...
class EuronewsParser(BaseParser):
    def extract(self, soup: BeautifulSoup, article_url: str) -> Optional[ParseResult]:
        ...
        meta_author = soup.find("meta", {"name": "article:author"}) or soup.find(
        ...
        if not author or not published_at:
            ...
            byline_div_published = soup.find(
                "div", {"class": "c-article-publication-date"}
            )
            ...
            if byline_div_author:
                ...
                if not author:
                    ...
                    full_byline_author_text = byline_div_author.get_text(
                        " ", strip=True
                    )
            ...
            if byline_div_published:
                ...
                if not published_at:
                    ...
                    if "Published on" in full_byline_published_text:
                        full_byline_published_text = full_byline_published_text.replace(
                            "Published on", ""
                        ).strip()
