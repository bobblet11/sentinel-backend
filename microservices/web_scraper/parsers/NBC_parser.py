...
class NBCParser(BaseParser):
    def extract(self, soup: BeautifulSoup, article_url: str) -> Optional[ParseResult]:
        container = soup.find("article", {"class": "article"}) or soup
        ...
        if not author or not published_at:
            ...
            byline_published_div = soup.find(
                "div", {"data-testid": "article-body-timestamp"}
            )
            ...
            if byline_author_spans:
                ...
                if not author:
                    author = ", ".join(
                        [a.get_text(strip=True) for a in byline_author_spans]
                    )
