...
class NBCParser(BaseParser):
    def extract(self, soup: BeautifulSoup, article_url: str) -> Optional[ParseResult]:
        ...
        if not author or not published_at:
            byline_author_spans = soup.find_all("span", {"class": "byline-name"})
            byline_published_div = soup.find("div", {"data-testid": "article-body-timestamp"})
            
            if byline_author_spans:
                # --- DOM Author Extraction ---
                if not author:
                    author = ", ".join([a.get_text(strip=True) for a in byline_author_spans])
            ...
            if byline_published_div:
                ...
                if not published_at:
                    ...
                    full_byline_published_text = byline_published_div.get_text(" ", strip=True)
