...
from microservices.web_scraper.config import PROXY_VALIDATION_MAX_WORKERS, WEBSHARIO_URL
from microservices.web_scraper.proxy_sources.base_classes import HttpProxySource
...
class WebshareIOHttpSource(HttpProxySource):
    ...
    def get_proxies(self, bootstrap_proxies: ProxyRequestDict = None) -> ProxyDict:
        ...
        def parse_line_fun(text: str):
            ...
            return [
                normalize_proxy_scheme_webshare_io(line.strip())
                for line in lines
                if line.strip()
            ]
        ...
        https = self._fetch_from_url(f"{self.BASE_URL}", bootstrap_proxies, line_parser)
    ...
    def create_ip_country_mapping(self, proxies) -> Dict[str, str]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=PROXY_VALIDATION_MAX_WORKERS
        ) as executor:
