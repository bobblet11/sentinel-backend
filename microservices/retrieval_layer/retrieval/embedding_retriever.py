...
def retrieve_by_embedding(
    ...
    stmt = stmt.order_by(
        Claim.decontextualised_embedding.cosine_distance(query_vec)
    ).limit(top_k)
    ...
    for row in results:
        ...
        source_excerpt = claim_excerpt[:300] + (
            "..." if len(claim_excerpt) > 300 else ""
        )
        claim_dict = {
            "id": row.id,
...
def retrieve_by_embedding_full_scan(
    ...
    stmt = (
        select(
            ...
            Claim.article_id,
            Article.url,
            Claim.decontextualised_embedding.cosine_distance(query_vec).label(
                "distance"
            ),
        )
        .join(Article, Article.id == Claim.article_id)
