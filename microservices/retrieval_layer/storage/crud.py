...
def create_sentiment(
    db: Session, sentiment_dto: CreateOrModifySentiment
) -> SentimentAnalysis:
...
def get_or_create_outlet(db: Session, outlet_dto: CreateOrModifyOutlet) -> NewsOutlet:
    ...
    existing_outlet_entry = db.execute(
        select(NewsOutlet).where(NewsOutlet.name == outlet_dto.name)
    ).scalar_one_or_none()
...
def get_or_create_article(
    ...
    outlet_dto: CreateOrModifyOutlet,
) -> Article:
...
def extend_evidence_claims_into_articles(
    db: Session, claim_ids: List[int], current_article_id: int
) -> List[Dict[str, Any]]:
    """
    ...
    for article in articles:
        ...
        bias_category = (
            article.sentiment.bias_category
            if article.sentiment and article.sentiment.bias_category
            else "center"
        )

        if bias_category.lower() not in [
            "left",
            "center-left",
            "center",
            "center-right",
...
def finalise_and_complete_job(db: Session, job_dto: UpdateJob):
    ...
    query_to_find_job = select(Job).where(
        Job.id == job_dto.job_id, Job.uid == job_dto.job_uid
    )
    ...
    existing_stage_names = set(
        db.execute(
            select(JobTimestamp.stage_name).where(
                JobTimestamp.job_id == existing_job.id
            )
        )
        .scalars()
        .all()
