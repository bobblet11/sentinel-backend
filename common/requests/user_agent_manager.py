...
class BrowserProfile:
    ...
    system_information: str = (
        "X11; Linux x86_64"  # "X11; Linux x86_64" is the standard identifier for Chrome on Ubuntu
    )
    platform_engine: str = (
        "AppleWebKit/537.36"  # Engine versions rarely change in modern Chrome strings
    )
    rendering_engine: str = (
        "KHTML, like Gecko"  # Engine versions rarely change in modern Chrome strings
    )
    compatibility_suffix: str = (
        "Safari/537.36"  # The suffix that mimics Safari (standard behavior)
    )
