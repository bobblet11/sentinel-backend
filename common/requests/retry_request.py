            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt == max_attempts:
                        logger.warning(
                            f"Attempted all {max_attempts} attempts. Exiting..."
                        )
                        raise e
                    logger.warning(
                        f"[Retry] Failure at attempt {attempt} doing '{func.__name__}' with error {type(e).__name__}. Will retry in {delay_s:.2f} s"
                    )
                    time.sleep(delay_s)
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    logger.warning(
                        f"[Retry] Failure at attempt {attempt} doing '{func.__name__}' with error {type(e).__name__}. Will retry in {time_to_wait_s:.2f} s"
                    )
