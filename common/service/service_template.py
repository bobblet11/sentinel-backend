...
class ServiceConfig:
    ...
    failure_output_stream: (
        str | None
    )  # if none, then failed batches will just not be published and will be pending for next cycle
...
class ServiceTemplate(ABC):
    ...
    def __init__(self, config: ServiceConfig) -> None:
        ...
        if config.input_streams:
            ...
            stream_to_priority_map = (
                PrioritisedRedisConsumerCombiner.generate_stream_to_priority_mapping(
                    config.input_streams
                )
            )
            stream_to_block_map = (
                PrioritisedRedisConsumerCombiner.generate_stream_to_block_mapping(
                    input_streams=config.input_streams,
                    level=config.block_prioritisation_level,
                )
            )
        ...
        if config.output_streams:
            ...
            routing_map = RedisPublisherRouter.generate_router_mapping(
                config.output_streams, config.router_key_values
            )
    ...
    def _acknowledge_failed_message(
        self, stream_name: str, redis_id: str, delete_from_stream: bool
    ) -> None:
        ...
        if delete_from_stream:
            self.message_consumer.acknowledge_and_delete(
                stream_name=stream_name, redis_message_id=redis_id
            )
        else:
    ...
    def _handle_failure(
        self, message: StreamMessage | Dict[str, Any], error: Exception
    ):
        ...
        redis_id = (
            ...
            else message.get("redis_message_id", "N/A")
        )
        stream_name = (
            message.stream if is_stream_message else message.get("stream", "N/A")
        )
    ...
    def _handle_failure_batch(
        self, messages: List[StreamMessage | Dict[str, Any]], error: Exception
    ):
        """Logs the error and publishes the message to the failure stream."""
        if not messages:
            ...
            return
        self.logger.error(
            f"Failed to process batch of {len(messages)} messages: {error}"
        )
    ...
    def _parse_message(self, raw_msg: Dict[str, Any]) -> Optional[StreamMessage]:
        ...
        try:
            ...
            self.logger.error(
                f"Pydantic validation failed for message {raw_msg['redis_message_id']}: {e}"
            )
    ...
    def _process_batch_sequentially(self, raw_messages: List[Dict[str, Any]]) -> None:
        ...
        for payload_id, original_message in payload_to_message_map.items():
            if payload_id in unroutable_payload_ids:
                ...
                if self.is_cut_and_paste_mode:
                    self.message_consumer.acknowledge_and_delete(
                        original_message.stream, original_message.redis_id
                    )
                else:
                    self.message_consumer.acknowledge(
                        message.stream, original_message.redis_id
                    )
        ...
        if failure_count > 0:
            self.logger.warning(
                f"Sent {failure_count} unroutable messages to failure stream."
            )
    ...
    def _process_batch_concurrently(
        ...
        for future in as_completed(future_to_message):
            ...
            try:
                ...
                self.logger.debug(
                    f"Successfully published Msg {old_redis_id} -> {new_redis_id}"
                )
            except Exception:
                self.logger.warning(
    ...
    def _get_raw_messages(self) -> List[Dict[str, Any]]:
        ...
        while self.keep_running:
            raw_messages = self.message_consumer.consume_many(
                num_to_consume=self.batch_size
            )
            if raw_messages:
                return raw_messages
        return []
    ...
    def _process_a_batch(self, executor: Optional[ThreadPoolExecutor]):
        ...
        if not raw_messages or len(raw_messages) == 0:
            self.logger.warning(
                "NO STREAM MESSAGES, sleeping for a minute before retrying..."
            )
            time.sleep(60)
            return
    ...
    def run(self):
        ...
        self.logger.info(
            f"Service '{self.__class__.__name__}' started. Listening on {self.input_streams}."
        )
        self.logger.info(
            f"Mode: {'Concurrent' if self.is_concurrent else 'Sequential'}."
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            try:
                ...
                self.logger.critical(
                    f"Unexpected critical error in main loop: {e}", exc_info=True
                )
            finally:
