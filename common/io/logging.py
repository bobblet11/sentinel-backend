...
def _cleanup_old_logs(log_directory: Path, max_age_of_log_file: TimeDeltaConfig):
    ...
    cutoff_datetime = datetime.datetime.utcnow() - datetime.timedelta(
        **asdict(max_age_of_log_file)
    )
...
def setup_logging(
    level=logging.INFO,
    container_name: str = "unknown_service",
    log_directory: Path = Path("/app/logs"),
    execution_mode: str = "long_running",
    max_age_of_log_file: TimeDeltaConfig = TimeDeltaConfig(),
):
    """
    ...
    if execution_mode == "cron":
        ...
        file_handler = TimedRotatingFileHandler(
            str(log_filepath), when="D", interval=1, backupCount=backup_count, utc=True
        )
    else:
        raise ValueError(
            f"Invalid execution_mode: '{execution_mode}'. Must be 'long_running' or 'cron'."
        )
    ...
    log_format = (
        f"%(asctime)s - %(levelname)s - [{container_name}.%(name)s] - %(message)s"
    )
...
def get_logger(func: Callable, args: tuple) -> logging.Logger:
    ...
    if (
        ...
        and isinstance(args[0].logger, logging.Logger)
    ):
