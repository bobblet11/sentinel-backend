...
_VALID_TOPICS = set(TOPIC_LABELS)
...
def validate(message: StreamMessage) -> Tuple[bool, List[str]]:
    ...
    # --- Topic classification (Stage 9) ---
    # topic_label is optional — TopicClassifier is non-fatal — so we warn rather
    # than hard-fail when the model was unavailable.  When a label IS present it
    # must be a known topic and carry a valid confidence score.
    if payload.topic_label is None:
        failures.append("topic_label is None (TopicClassifier may have been skipped)")
    else:
        if payload.topic_label not in _VALID_TOPICS:
            failures.append(
                f"topic_label '{payload.topic_label}' is not a recognised topic "
                f"(expected one of: {sorted(_VALID_TOPICS)})"
            )
        if payload.topic_confidence is None:
            failures.append("topic_confidence is None despite topic_label being set")
        elif not (0.0 <= payload.topic_confidence <= 1.0):
            failures.append(
                f"topic_confidence {payload.topic_confidence:.4f} is outside [0, 1]"
            )
