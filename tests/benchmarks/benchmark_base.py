...
SUBTASK_PAIRS = {
    ...
    "NLP.ENTITY_MAPPING": (
        JobStage.CHECK_WORTHY_ENTITY_MAPPING_IN,
        JobStage.CHECK_WORTHY_ENTITY_MAPPING_OUT,
    ),
    "NLP.SENTENCE_EMBED": (JobStage.SENTENCE_EMBED_IN, JobStage.SENTENCE_EMBED_OUT),
    "NLP.CONVERT_TO_CLAIM": (
        JobStage.CONVERT_TO_CLAIM_IN,
        JobStage.CONVERT_TO_CLAIM_OUT,
    ),
    ...
    "RETRIEVAL.RETRIEVE_EVIDENCE": (
        JobStage.RETRIEVE_EVIDENCE_IN,
        JobStage.RETRIEVE_EVIDENCE_OUT,
    ),
...
class GroupStats:
    ...
    name: str
    job_count: int = 0
    service_latencies: Dict[str, MetricStats] = field(
        default_factory=dict
    )  # NEW: Service-level
    subtask_latencies: Dict[str, MetricStats] = field(
        default_factory=dict
    )  # NEW: Subtask-level
...
class BenchmarkResults:
    ...
    def _get_metric_stats(
        self, values: List[float], jobs: List[Dict[str, Any]]
    ) -> MetricStats:
        if not values:
            return MetricStats()
    ...
    def _calculate_group_stats(
        self, jobs: List[Dict[str, Any]], group_name: str
    ) -> GroupStats:
        ...
        for job in jobs:
            ...
            for service_name, (start_stage, end_stage) in SERVICE_BOUNDARIES.items():
                ...
                if (
                    ...
                    service_duration = (
                        timestamps[end_idx]["offset_s"]
                        - timestamps[start_idx]["offset_s"]
                    )
            ...
            for subtask_name, (in_stage, out_stage) in SUBTASK_PAIRS.items():
                ...
                out_idx = next(
                    (
                        i
                        for i, ts in enumerate(timestamps)
                        if ts["stage_name"] == out_stage
                    ),
                    None,
                )
                if in_idx is not None and out_idx is not None and out_idx > in_idx:
                    subtask_duration = (
                        timestamps[out_idx]["offset_s"] - timestamps[in_idx]["offset_s"]
                    )
                    durations.setdefault(subtask_name, []).append(subtask_duration)

        stats = GroupStats(
            name=group_name, job_count=len(valid_jobs), raw_durations=durations
        )

        if valid_jobs:
            # Overall end-to-end
            stats.end_to_end = self._get_metric_stats(
                durations["end_to_end"], valid_jobs
            )

            # SERVICE latencies (exactly what you want #1)
            for service_name, values in durations.items():
                if service_name in SERVICE_BOUNDARIES:
                    stats.service_latencies[service_name] = self._get_metric_stats(
                        values, valid_jobs
                    )
            ...
            for subtask_name, values in durations.items():
                if subtask_name in SUBTASK_PAIRS:
                    stats.subtask_latencies[subtask_name] = self._get_metric_stats(
                        values, valid_jobs
                    )
    ...
    def calculate_statistics(self) -> Dict[str, GroupStats]:
        ...
        user_jobs, background_jobs, all_jobs = (
            [],
            [],
            self.successfully_processed_results,
        )
    ...
    def save_json(self, filepath: str) -> Path:
        ...
        for group_name, group_stats in stats.items():
            serializable["statistics"][group_name] = {
                ...
                "service_latencies": {
                    k: asdict(v) for k, v in group_stats.service_latencies.items()
                },
                "subtask_latencies": {
                    k: asdict(v) for k, v in group_stats.subtask_latencies.items()
                },
    ...
    def print_report(self) -> None:
        ...
        for group_name, group_stats in stats_dict.items():
            ...
            print(
                f"End-to-End: avg={group_stats.end_to_end.avg:.2f}s, "
            ...
            if group_stats.service_latencies:
                ...
                for service_name, metric in sorted(
                    group_stats.service_latencies.items()
                ):
                    print(
                        f"  {service_name:15}: {metric.avg:6.2f}s / p95:{metric.p95:6.2f}s"
                    )
            ...
            if group_stats.subtask_latencies:
                ...
                for subtask_name, metric in top_subtasks:
                    ...
                    print(
                        f"  {service:12} | {subtask_name:25}: {metric.avg:5.2f}s / p95:{metric.p95:5.2f}s"
                    )
...
class BenchmarkTemplate(ABC):
    ...
    def setup(self):
        ...
        for stream in target_streams:
            ...
            try:
                ...
                assert any(
                    g["name"] == self.consumer_group for g in groups
                ), f"Group missing on {stream}"
    ...
    # job_create
    def _create_api_job(
        self, article_url: str, is_background: bool = True
    ) -> Dict[str, Any]:
        if not article_url:
            raise Exception(
                "article url missing, minumum data required is a article url"
            )
        return {"article_url": article_url, "is_background": is_background}

    # payload
    def _create_redis_job(
        self, article_url: str, news_outlet: str = None, is_background: bool = True
    ) -> Message:
        ...
        payload = MessagePayload(
            article_url=article_url, news_outlet=news_outlet, title=None, summary=None
        )
    ...
    def _submit_job_redis(self, payload: Message) -> Dict[str, Any]:
        try:
            ...
            redis_id = self.redis.xadd(
                self.input_stream, {"payload": payload_str}, approximate=True
            )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for job in jobs:
                if job["submission_type"] == "redis":
                    futures[executor.submit(self._submit_job_redis, job["payload"])] = (
                        job
                    )
                else:
    ...
    def _extract_uid(self, submission_result: Dict[str, Any]) -> Optional[str]:
        ...
        if "payload" in submission_result and isinstance(
            submission_result["payload"], Message
        ):
    ...
    def _poll_results(
        ...
        while len(results) < len(expected) and (time.time() - start) < timeout_s:
            if poll_count % 10 == 0:
                ...
                print(
                    f"Poll {poll_count}: {len(results)}/{len(expected)} ({remaining} remaining)"
                )

            streams_dict = {stream: ">" for stream in target_streams}

            print(
                f"\t[POLL] Iteration={poll_count}, Streams={streams_dict}, ResultsSoFar={len(results)}"
            )
            ...
            for stream_name_bytes, messages in response:
                ...
                for redis_message_id_bytes, fields in messages:
                    ...
                    try:
                        message_dict = self._decode_one_message(
    ...
    def run(self) -> BenchmarkResults:
        ...
        print(
            f"Submitted {len(submit_successes)} jobs successfully, {len(submit_failures)} failed"
        )

        # Extract UIDs
        expected_uids = [
            self._extract_uid(s) for s in submit_successes if self._extract_uid(s)
        ]
        ...
        successfully_processed_jobs = [
            j for _, j in results.items() if not j.get("is_failure")
        ]
