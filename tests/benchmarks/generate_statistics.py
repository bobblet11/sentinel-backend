...
class Group:
    ...
    def calculate_statistics(self) -> StatisticForGroup:
        ...
        for job in self.jobs:
            ...
            durations["bench_time"].append(
                timestamps[-1]["offset_s"] - timestamps[0]["offset_s"]
            )
            ...
            durations["fetch_time"].append(
                timestamps[3]["offset_s"] - timestamps[2]["offset_s"]
            )
            durations["parse_time"].append(
                timestamps[5]["offset_s"] - timestamps[4]["offset_s"]
            )
        ...
        def get_metric_stats(values: List[float], jobs: List[Dict]) -> MetricStats:
            ...
            def find_closest_job(target_val):
                closest_index = min(
                    range(len(values)), key=lambda i: abs(values[i] - target_val)
                )
...
def read_and_segment_groups(result_path: Path) -> Tuple[Group, Group, Group]:
    ...
    return (
        Group("COMBINED", all_jobs),
        Group("USER", user_jobs),
        Group("BACKGROUND", background_jobs),
    )
...
def generate_charts(
    user_stats: StatisticForGroup,
    background_stats: StatisticForGroup,
    base_filename: str,
):
    ...
    df = pd.DataFrame(
        {
            ...
            "Average End-to-End Time (s)": [
                user_stats.total_bench_time.avg,
                background_stats.total_bench_time.avg,
            ],
            "Average Wait Time (s)": [
                user_stats.total_wait_time.avg,
                background_stats.total_wait_time.avg,
            ],
        }
    )
    df.plot(
        x="Group",
        y=["Average End-to-End Time (s)", "Average Wait Time (s)"],
        kind="bar",
        figsize=(10, 7),
        rot=0,
    )
    plt.title(
        f"Performance Comparison: User vs. Background\n({base_filename})", fontsize=16
    )
    ...
    plt.boxplot(
        [
            user_stats.raw_durations["bench_time"],
            background_stats.raw_durations["bench_time"],
        ],
    ...
    ax.bar(
        labels,
        parse_times,
        bottom=np.array(wait_times) + np.array(fetch_times),
        label="Parse Time",
    )
...
if __name__ == "__main__":
    ...
    if not benchmark_files:
        print(
            f"No benchmark files found in '{LOGS_DIRECTORY}' matching the pattern 'benchmark_*_*_timestamps.json'."
        )
        print(
            "Please ensure your files are in the correct directory and you have run 'pip install matplotlib pandas numpy'."
        )
